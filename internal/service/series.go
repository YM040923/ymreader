...
func naturalLess(a, b string) bool {
	...
	for ia, ib := 0, 0; ia < len(ra) && ib < len(rb); {
		na, ja, aIsNumber := readNaturalNumber(ra, ia)
		nb, jb, bIsNumber := readNaturalNumber(rb, ib)
...
func readNaturalNumber(value []rune, start int) (uint64, int, bool) {
	if start >= len(value) {
		return 0, start, false
	}
	if unicode.IsDigit(value[start]) {
		end := start
		for end < len(value) && unicode.IsDigit(value[end]) {
			end++
		}
		number, err := strconv.ParseUint(string(value[start:end]), 10, 64)
		return number, end, err == nil
	}
	if !isChineseNumberRune(value[start]) {
		return 0, start, false
	}
	end := start
	for end < len(value) && isChineseNumberRune(value[end]) {
		end++
	}
	number, ok := parseChineseNaturalNumber(string(value[start:end]))
	return number, end, ok
}

func isChineseNumberRune(value rune) bool {
	return strings.ContainsRune("零〇一二两三四五六七八九十百千万", value)
}

func parseChineseNaturalNumber(value string) (uint64, bool) {
	digits := map[rune]uint64{
		'零': 0, '〇': 0, '一': 1, '二': 2, '两': 2,
		'三': 3, '四': 4, '五': 5, '六': 6, '七': 7, '八': 8, '九': 9,
	}
	var total, section, current uint64
	seen := false
	...
	return total + section + current, seen
}
