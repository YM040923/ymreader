...
// ============================================================
// Unified scan-rule engine (AI inference plus optional physical directory organization).
//
// 设计原则：
//   1. 默认全关，用户在设置页主动启用
...
func RunScanRules(opts ScanRuleRunOptions) (*ScanRuleRunResult, error) {
	...
	result.Duration = time.Since(startedAt).Milliseconds()
	log.Printf("[scan-rules] batch=%s done in %dms inferred=%d organized=%d skipped=%d failed=%d",
		opts.BatchID, result.Duration, result.Inferred, result.DirectoryOrganized, result.Skipped, result.Failed)
	return result, nil
}
...
// Then write the inferred metadata back to each configured Comic target.
