...
func TestOPDSBasicAPIKeyCanFetchSeriesWorkCoverDirectly(t *testing.T) {
	setupTestRouter(t)
	if err := store.RunMigrations(); err != nil {
		t.Fatalf("RunMigrations failed: %v", err)
	}
	user, token := createOPDSTestUserAndKey(t, "opds-series-cover-user", "opds-series-cover-user")
	libraryDir := t.TempDir()
	library := &model.Library{
		ID: "opds-series-cover-lib", Name: "opds-series-cover-lib", Type: "comic",
		RootPath: libraryDir, Enabled: true, DefaultAccess: "private", ScanEnabled: true,
	}
	if err := store.CreateLibrary(library); err != nil {
		t.Fatalf("CreateLibrary failed: %v", err)
	}
	const comicID = "opds-series-cover-comic"
	const filename = "Series Cover/Chapter 001.cbz"
	archivePath := filepath.Join(libraryDir, filepath.FromSlash(filename))
	if err := os.MkdirAll(filepath.Dir(archivePath), 0o755); err != nil {
		t.Fatalf("create series cover directory: %v", err)
	}
	if err := os.WriteFile(archivePath, createImageCBZ(t), 0o600); err != nil {
		t.Fatalf("write series cover CBZ: %v", err)
	}
	...
	if _, err := store.DB().Exec(`UPDATE "Comic" SET "pageCount" = 1 WHERE "id" = ?`, comicID); err != nil {
		t.Fatalf("update series cover comic: %v", err)
	}
	...
	handler.workLoader = func(c *gin.Context) ([]opdsWorkCatalogItem, error) {
		return []opdsWorkCatalogItem{{
			Work: service.Work{
				ID: "opds-series-cover-work", LibraryID: library.ID,
				SeriesID: "opds-series-cover", CoverComicID: comicID,
			},
		}}, nil
	}
	router := gin.New()
	opds := router.Group("/api/opds")
	opds.Use(middleware.OPDSAuthRequired())
	opds.GET("/work-cover/:id", handler.WorkCover)

	cover := performOPDSBasicRequest(
		router,
		"/api/opds/work-cover/opds-series-cover-work",
		user.Username,
		token,
	)
	if cover.Code != http.StatusOK {
		t.Fatalf("Basic-auth Work cover returned %d: %s", cover.Code, cover.Body.String())
	}
	if location := cover.Header().Get("Location"); location != "" {
		t.Fatalf("OPDS Work cover redirected to %q instead of returning image bytes", location)
	}
	if cacheControl := cover.Header().Get("Cache-Control"); !strings.HasPrefix(cacheControl, "private,") {
		t.Fatalf("Work cover Cache-Control = %q, want private cache", cacheControl)
	}
	vary := cover.Header().Get("Vary")
	if !strings.Contains(vary, "Authorization") || !strings.Contains(vary, "Cookie") {
		t.Fatalf("Work cover Vary = %q, want Authorization and Cookie", vary)
	}
	if contentType := cover.Header().Get("Content-Type"); !strings.HasPrefix(contentType, "image/") {
		t.Fatalf("Work cover Content-Type = %q, want image", contentType)
	}
}
...
func TestOPDSFeedRequiresDownloadAccessAndExcludesNovelLibraries(t *testing.T) {
	...
	response := performOPDSBasicRequest(router, "/api/opds/recent?page=1&pageSize=10", user.Username, token)
	if response.Code != http.StatusOK {
		t.Fatalf("OPDS recent returned %d: %s", response.Code, response.Body.String())
	}
	if contentType := response.Header().Get("Content-Type"); !strings.HasPrefix(contentType, service.OPDSNavigationMIME) {
		t.Fatalf("recent Content-Type = %q, want Work navigation %q", contentType, service.OPDSNavigationMIME)
	}
	if cacheControl := response.Header().Get("Cache-Control"); cacheControl != "private, no-store" {
		t.Fatalf("recent Cache-Control = %q, want private, no-store", cacheControl)
	}
	body := response.Body.String()
	if strings.Count(body, "<entry>") != 1 || !strings.Contains(body, "/api/opds/works/") {
		t.Fatalf("recent did not return one downloadable comic Work: %s", body)
	}
	for _, forbidden := range []string{"opds-view-comic", "opds-novel", "opds-download-ebook"} {
	...
	works, err := loadOPDSWorksFromResponseFixture(user.ID, "opds-download-lib")
	if err != nil || len(works) != 1 {
		t.Fatalf("build recent Work fixture: %v, count=%d", err, len(works))
	}
	detail := performOPDSBasicRequest(router, "/api/opds/works/"+works[0].ID, user.Username, token)
	if detail.Code != http.StatusOK {
		t.Fatalf("recent Work detail returned %d: %s", detail.Code, detail.Body.String())
	}
	if contentType := detail.Header().Get("Content-Type"); !strings.HasPrefix(contentType, service.OPDSAcquisitionMIME) {
		t.Fatalf("Work detail Content-Type = %q, want acquisition", contentType)
	}
	if !strings.Contains(detail.Body.String(), `/api/opds/stream/opds-download-comic?`) {
		t.Fatalf("comic Unit is missing OPDS-PSE link: %s", detail.Body.String())
	}
...
func TestOPDSPSEStreamsJPEGWithDownloadPermission(t *testing.T) {
	...
	t.Cleanup(func() {
		service.InvalidateContentFileCaches("opds-pse-comic", archivePath)
	})
	...
	feed := performOPDSBasicRequest(router, "/api/opds/recent", user.Username, token)
	if feed.Code != http.StatusOK {
		t.Fatalf("PSE feed returned %d: %s", feed.Code, feed.Body.String())
	}
	if contentType := feed.Header().Get("Content-Type"); !strings.HasPrefix(contentType, service.OPDSNavigationMIME) {
		t.Fatalf("PSE recent Content-Type = %q, want Work navigation", contentType)
	}
	works, err := loadOPDSWorksFromResponseFixture(user.ID, library.ID)
	if err != nil || len(works) != 1 {
		t.Fatalf("build PSE Work fixture: %v, count=%d", err, len(works))
	}
	feed = performOPDSBasicRequest(router, "/api/opds/works/"+works[0].ID, user.Username, token)
	if feed.Code != http.StatusOK {
		t.Fatalf("PSE Work detail returned %d: %s", feed.Code, feed.Body.String())
	}
...
func TestOPDSSeriesFeedsAreDownloadScopedAndFlattenSections(t *testing.T) {
	...
	createOPDSHandlerComic(t, "opds-series-v1", "Work/Chapter 01.cbz", "comic", "opds-series-download")
	createOPDSHandlerComic(t, "opds-series-v2", "Work/Chapter 02.pdf", "comic", "opds-series-download")
	createOPDSHandlerComic(t, "opds-view-v1", "Private/Chapter 01.cbz", "comic", "opds-series-view")
	createOPDSHandlerComic(t, "opds-view-v2", "Private/Chapter 02.cbz", "comic", "opds-series-view")
	...
	if strings.Count(list.Body.String(), "<entry>") != 1 ||
		!strings.Contains(list.Body.String(), "<title>Work</title>") ||
		strings.Contains(list.Body.String(), "<title>Private</title>") {
		t.Fatalf("series compatibility catalog did not expose one download-scoped Work: %s", list.Body.String())
	}
	...
	if strings.Count(all.Body.String(), "<entry>") != 1 || !strings.Contains(all.Body.String(), "<title>Work</title>") {
		t.Fatalf("all and series did not expose the same unified Work catalog: %s", all.Body.String())
	}
