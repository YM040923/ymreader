...
func registerMetadataRoutes(api *gin.RouterGroup) {
	...
	metadataGroup.Use(middleware.AdminRequired(), middleware.ScraperRequired())
	{
		...
		metadataGroup.POST("/tasks/libraries/:id", meta.StartLibraryWorkScrapeTask)
		metadataGroup.GET("/tasks/:id", meta.GetWorkScrapeTask)
		metadataGroup.POST("/tasks/:id/cancel", meta.CancelWorkScrapeTask)
	...
	opdsGroup.Use(suppressOPDSHeadResponseBody(), middleware.OPDSAuthRequired())
	{
		...
		opdsGroup.GET("/work-cover/:id", opds.WorkCoverFast)
		opdsGroup.HEAD("/work-cover/:id", opds.WorkCoverFast)
