...
func registerLibraryRoutes(api *gin.RouterGroup) {
	...
	libraryGroup.Use(middleware.LibraryTypeGuard())
	{
		...
		libraryGroup.POST("/:id/scan", reconcileOwnershipAfterScan(), rebuildSeriesAfterScan(), rebuildWorksAfterScan(), library.ScanLibrary)
	...
	accessibleGroup.Use(middleware.AuthRequired())
	{
		...
		accessibleGroup.POST("/:id/scan", reconcileOwnershipAfterScan(), rebuildSeriesAfterScan(), rebuildWorksAfterScan(), library.ScanLibrary)
	}
