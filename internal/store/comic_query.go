...
// GetWorkSourceFingerprint returns a cheap change token for the data used to
// build a user's Work catalog. It covers physical Comic metadata, user state,
// tags/categories and persisted ComicSeries metadata.
func GetWorkSourceFingerprint(userID string, libraryIDs []string, filterLibraryIDs bool) (string, error) {
	conditions := []string{`c."type" = 'comic'`}
	args := []interface{}{}
	if filterLibraryIDs {
		if len(libraryIDs) == 0 {
	...
	var comicCount, comicShape int64
	var comicUpdated string
	if err := db.QueryRow(`
		SELECT COUNT(*), COALESCE(MAX(CAST(c."updatedAt" AS TEXT)), ''),
		       COALESCE(SUM(
		         LENGTH(COALESCE(c."relativePath", c."filename")) +
		         LENGTH(COALESCE(c."title", '')) +
		         LENGTH(COALESCE(c."author", '')) +
		         LENGTH(COALESCE(c."publisher", '')) +
		         LENGTH(COALESCE(c."description", '')) +
		         LENGTH(COALESCE(c."genre", '')) +
		         LENGTH(COALESCE(c."metadataSource", '')) +
		         COALESCE(c."pageCount", 0) + COALESCE(c."sortOrder", 0)
		       ), 0)
		FROM "Comic" c `+where, args...).Scan(&comicCount, &comicUpdated, &comicShape); err != nil {
		return "", err
	}

	tagHash, err := orderedWorkContentHash(`
		SELECT CAST(c."id" AS TEXT), CAST(t."id" AS TEXT), t."name", t."color"
		FROM "ComicTag" relation
		JOIN "Comic" c ON c."id" = relation."comicId"
		JOIN "Tag" t ON t."id" = relation."tagId"
		`+where+` ORDER BY c."id", t."id"
	`, args, 4)
	if err != nil {
		return "", err
	}
	categoryHash, err := orderedWorkContentHash(`
		SELECT CAST(c."id" AS TEXT), CAST(category."id" AS TEXT),
		       category."name", category."slug", category."icon"
		FROM "ComicCategory" relation
		JOIN "Comic" c ON c."id" = relation."comicId"
		JOIN "Category" category ON category."id" = relation."categoryId"
		`+where+` ORDER BY c."id", category."id"
	`, args, 5)
	if err != nil {
		return "", err
	}

	var stateCount, stateShape int64
	var stateUpdated string
	if userID != "" {
		stateArgs := append([]interface{}{userID}, args...)
		if err := db.QueryRow(`
			SELECT COUNT(ucs."comicId"), COALESCE(MAX(CAST(ucs."lastReadAt" AS TEXT)), ''),
			       COALESCE(SUM(
			         COALESCE(ucs."lastReadPage", 0) + COALESCE(ucs."isFavorite", 0) +
			         COALESCE(ucs."rating", 0) + COALESCE(ucs."totalReadTime", 0) +
			         LENGTH(COALESCE(ucs."readingStatus", ''))
			       ), 0)
			FROM "Comic" c
			LEFT JOIN "UserComicState" ucs ON ucs."comicId" = c."id" AND ucs."userId" = ?
			`+where, stateArgs...).Scan(&stateCount, &stateUpdated, &stateShape); err != nil {
			return "", err
		}
	}

	seriesCount, seriesShape, seriesUpdated := int64(0), int64(0), ""
	seriesTagsHash := ""
	if workTableExists("ComicSeries") {
		seriesConditions := []string{"1=1"}
		seriesArgs := []interface{}{}
		if filterLibraryIDs {
			if len(libraryIDs) == 0 {
				seriesConditions = append(seriesConditions, "1=0")
			} else {
				placeholders := make([]string, len(libraryIDs))
				for index, id := range libraryIDs {
					placeholders[index] = "?"
					seriesArgs = append(seriesArgs, id)
				}
				seriesConditions = append(seriesConditions, fmt.Sprintf(`s."libraryId" IN (%s)`, strings.Join(placeholders, ",")))
			}
		}
		if err := db.QueryRow(`
			SELECT COUNT(*), COALESCE(MAX(CAST(s."updatedAt" AS TEXT)), ''),
			       COALESCE(SUM(
			         LENGTH(s."rootRelativePath") + LENGTH(s."title") + LENGTH(s."author") +
			         LENGTH(s."publisher") + LENGTH(s."description") + LENGTH(s."genre") +
			         LENGTH(s."coverUrl")
			       ), 0)
			FROM "ComicSeries" s
			WHERE `+strings.Join(seriesConditions, " AND "), seriesArgs...).Scan(
			&seriesCount, &seriesUpdated, &seriesShape,
		); err != nil {
			return "", err
		}
		if workTableExists("ComicSeriesTag") {
			seriesTagsHash, err = orderedWorkContentHash(`
				SELECT CAST(s."id" AS TEXT), CAST(t."id" AS TEXT), t."name", t."color"
				FROM "ComicSeriesTag" relation
				JOIN "ComicSeries" s ON s."id" = relation."seriesId"
				JOIN "Tag" t ON t."id" = relation."tagId"
...
func orderedWorkContentHash(query string, args []interface{}, columnCount int) (string, error) {
...
func workTableExists(name string) bool {
	var found string
...
func GetAllComics(opts ComicListOptions) (*ComicListResult, error) {
	...
	if opts.UserID != "" {
		...
		SELECT c."id", c."filename", c."title", c."titleSortKey", c."pageCount", c."fileSize",
		       ...
		       COALESCE(ucs."readingStatus", '') AS readingStatus, c."type",
		       COALESCE(c."libraryId", ''), c."coverImageUrl", c."coverAspectRatio",
		       c."externalRating", c."externalRatingMax", c."externalRatingSource", c."externalRatingUpdatedAt",
		       COALESCE(NULLIF(c."relativePath", ''), c."filename")
		FROM "Comic" c
		%s %s %s %s
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&extRating, &extRatingMax, &extRatingSource, &extRatingUpdatedAtStr, &c.RelativePath,
		); err != nil {
...
func GetComicByIDForUser(comicID string, userID string) (*ComicListItem, error) {
	...
	err := db.QueryRow(query, userID, comicID).Scan(
		...
		&extRating, &extRatingMax, &extRatingSource, &extRatingUpdatedAtStr, &ci.RelativePath,
	)
	if err == sql.ErrNoRows {
		return nil, nil
	}
