...
// ReadingSessionRecord is the lossless storage DTO used by higher-level
// Work-aware statistics. ComicID always remains the physical Comic row ID.
type ReadingSessionRecord struct {
	ID          int
	UserID      string
	ComicID     string
	ContentType string
...
// GetReadingSessionRecords returns lossless physical reading sessions for
// higher-level Work aggregation. Library filtering is applied before records
// leave the store so inaccessible titles cannot leak through statistics.
func GetReadingSessionRecords(userID string, libraryIDs []string, filterLibraryIDs bool) ([]ReadingSessionRecord, error) {
	conditions := []string{`rs."duration" > 0`}
	args := []interface{}{}
	if userID != "" {
		conditions = append(conditions, `rs."userId" = ?`)
		args = append(args, userID)
	}
	if filterLibraryIDs {
		if len(libraryIDs) == 0 {
			return []ReadingSessionRecord{}, nil
		}
		conditions = append(conditions, `c."libraryId" IN (`+placeholders(len(libraryIDs))+`)`)
	...
	rows, err := db.Query(`
		SELECT rs."id", rs."userId", rs."comicId", c."type", c."libraryId",
		       c."title", c."genre", rs."startedAt", rs."endedAt",
		       rs."duration", rs."startPage", rs."endPage"
		FROM "ReadingSession" rs
		JOIN "Comic" c ON c."id" = rs."comicId"
		WHERE `+strings.Join(conditions, " AND ")+`
		ORDER BY `+startedAtExpr+` DESC, rs."id" DESC
	`, args...)
	if err != nil {
		return nil, err
	}
	...
	result := []ReadingSessionRecord{}
	for rows.Next() {
		var record ReadingSessionRecord
		var endedAt sql.NullTime
		if err := rows.Scan(
			&record.ID, &record.UserID, &record.ComicID, &record.ContentType,
			&record.LibraryID, &record.Title, &record.Genre, &record.StartedAt,
			&endedAt, &record.Duration, &record.StartPage, &record.EndPage,
		); err != nil {
			return nil, err
		}
		record.CoverURL = BuildComicCoverURL(record.ComicID)
		if endedAt.Valid {
			value := endedAt.Time
