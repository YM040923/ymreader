...
type ScanRuleOpLog struct {
	...
	Action    string    `json:"action"`              // ai_infer | metadata_apply | directory_organize
...
func InsertScanRuleOpLog(log ScanRuleOpLog) error {
	...
	_, err := db.Exec(
		`INSERT INTO "ScanRuleOpLog" ("batchId","comicId","action","status","fromValue","toValue","message","createdAt")
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		log.BatchID, log.ComicID, log.Action, log.Status,
...
func ListScanRuleOpLogs(batchID string, limit int) ([]ScanRuleOpLog, error) {
	...
	for rows.Next() {
		var l ScanRuleOpLog
		if err := rows.Scan(&l.ID, &l.BatchID, &l.ComicID,
