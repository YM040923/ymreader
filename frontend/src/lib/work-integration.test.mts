...
assert.match(detail, /localStorage\.getItem\("work:unitView"\)/);
assert.match(detail, /setViewMode\("list"\)/);
assert.match(detail, /setViewMode\("grid"\)/);
assert.match(detail, /整部进度/);
assert.match(detail, /完成比例/);
assert.match(detail, /阅读时长/);
