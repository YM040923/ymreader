...
import { apiComicToComic, mergeNovelAndWorkItems, workToComic } from "@/lib/work-model";
import { fetchAllComics } from "@/api/catalogContent";
...
export default function HistoryPage() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const [works, novels] = await Promise.all([
        fetchAllWorks({ sortBy: "lastReadAt", sortOrder: "desc" }),
        fetchAllComics({ contentType: "novel", sortBy: "lastReadAt", sortOrder: "desc" }),
      ]);
      setComics(
        mergeNovelAndWorkItems(
          novels.filter((novel) => Boolean(novel.lastReadAt)).map(apiComicToComic),
          works.filter((work) => Boolean(work.lastReadAt)).map(workToComic),
        ).sort((left, right) =>
          new Date(right.lastReadAt || 0).getTime()
          - new Date(left.lastReadAt || 0).getTime(),
        ),
      );
    } catch {
      // silent
  ...
  const filtered = useMemo(() => {
    ...
    else if (activeFilter === "reading") items = items.filter((c) => !isStoredReadingFinished(c.lastReadPage || 0, c.pageCount || 0, c.lastReadAt || null, c.readingStatus));
    else if (activeFilter === "finished") items = items.filter((c) => isStoredReadingFinished(c.lastReadPage || 0, c.pageCount || 0, c.lastReadAt || null, c.readingStatus));
    ...
    if (activeSort === "recent") {
      ...
      items.sort((a, b) => calculateStoredReadingProgress(b.lastReadPage || 0, b.pageCount || 0, b.lastReadAt || null, b.readingStatus) - calculateStoredReadingProgress(a.lastReadPage || 0, a.pageCount || 0, a.lastReadAt || null, a.readingStatus));
    }
    return items;
  ...
  const grouped = useMemo<HistoryGroup[]>(() => {
    ...
    for (const c of filtered) {
      const g = getDateGroup(c.lastReadAt || null);
  ...
  const summary = useMemo(() => {
    ...
    const readingCount = comics.filter((c) => !isStoredReadingFinished(c.lastReadPage || 0, c.pageCount || 0, c.lastReadAt || null, c.readingStatus)).length;
    const finishedCount = comics.filter((c) => isStoredReadingFinished(c.lastReadPage || 0, c.pageCount || 0, c.lastReadAt || null, c.readingStatus)).length;
...
function HistoryCard({ comic }: { comic: Comic }) {
  ...
  const hasStarted = !!comic.lastReadAt || (comic.lastReadPage || 0) > 0;
  ...
  return (
    <div className="group flex gap-3 sm:gap-4 rounded-2xl border border-border/30 bg-card/60 backdrop-blur-sm p-3 sm:p-4 transition-all hover:border-border/60 hover:bg-card/80">
      {/* Cover */}
      <Link
        href={comic.detailHref || `/work/${comic.id}`}
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <Link href={comic.detailHref || `/work/${comic.id}`} className="text-sm font-semibold text-foreground hover:text-accent transition-colors truncate">
        ...
        <div className="flex items-center gap-3 mt-2">
          ...
          {(comic.pageCount || 0) > 0 && (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <span className="text-[10px] text-muted shrink-0">
                {getReadingPageNumber(comic.lastReadPage || 0, comic.pageCount || 0)}/{comic.pageCount || 0} 页
              </span>
            </div>
          )}

          {(comic.totalReadTime || 0) > 0 && (
          ...
          <div className="flex items-center gap-1.5 ml-auto shrink-0">
            ...
            {!finished && (comic.pageCount || 0) > 0 && (
              <Link
                href={comic.readHref || comic.detailHref || `/work/${comic.id}`}
