...
import { parseReaderNavigation } from "@/lib/work-model";
...
export default function ReaderPage() {
  ...
  const physicalPages = useMemo(
    () => pdfTotalPages && usePdfView
      ? Array.from(
        { length: pdfTotalPages },
        (_, i) => apiPath(`/api/comics/${comicId}/page/${i}`),
      )
      : apiPages,
    [apiPages, comicId, pdfTotalPages, usePdfView],
  );
  const activeUnit = useMemo(
    () => readerWork
      ? findActiveWorkUnit(readerWork, readerNavigation, comicId)
      : null,
    [comicId, readerNavigation, readerWork],
  );
  const pages = useMemo(
    () => slicePagesForUnit(physicalPages, activeUnit),
    [activeUnit, physicalPages],
  );
  const readerWindowReady = !readerNavigation.workId || activeUnit !== null;
  const unitResolutionError =
    readerWork && readerNavigation.workId && !activeUnit
      ? "Requested unit does not belong to this comic"
      : null;
  const title = activeUnit?.title || readerWork?.title || apiTitle || t.reader.unknownComic;
  const isLoading = apiLoading || workLoading;
  ...
  useEffect(() => {
    ...
    setWorkLoading(true);
    setWorkError(null);
    fetchWork(readerNavigation.workId)
      .then((work) => {
        if (!cancelled) setReaderWork(work);
      })
      .catch((error) => {
        if (!cancelled) {
          setReaderWork(null);
          setWorkError(
            error instanceof Error ? error.message : "Work not found",
          );
        }
      })
      .finally(() => {
  ...
  const appliedWorkNavigationRef = useRef("");
  ...
  useEffect(() => {
    if (comicDetail && useRealData && readerWindowReady) {
      const navigationKey = `${comicId}:${location.search}`;
      // Work 目录/继续阅读传入的是物理页，Reader 内部始终使用 Unit 相对页。
      if (readerNavigation.startPage !== null) {
        if (
          pages.length > 0
          && appliedWorkNavigationRef.current !== navigationKey
        ) {
          const requestedPage = activeUnit
            ? relativePageForUnit(activeUnit, readerNavigation.startPage)
            : Math.max(0, Math.min(pages.length - 1, readerNavigation.startPage));
        ...
        && comicDetail.lastReadPage > 0
        && comicDetail.lastReadPage < pages.length
      ) {
  ...
  useEffect(() => {
    ...
    setSeriesVolumes(
      [...readerWork.units].sort((left, right) => left.sortIndex - right.sortIndex),
    );
    setSeriesName(readerWork.title);
  }, [readerWork]);
    ...
    (e: KeyboardEvent) => {
      ...
      if (isForward || e.key === "ArrowDown" || e.key === " ") {
        ...
        if (showInfoPanel) {
          ...
          finishSessionRef.current?.();
          if (readerNavigation.workId) {
            router.replace(`/work/${encodeURIComponent(readerNavigation.workId)}`);
          } else {
            router.back();
          }
        }
  ...
  const handleOpenComicDetail = useCallback(async () => {
    ...
    if (readerNavigation.workId) {
      router.push(`/work/${encodeURIComponent(readerNavigation.workId)}`);
    } else {
      router.push(`/comic/${comicId}`);
    }
  }, [comicId, readerNavigation.workId, router]);
  ...
  if (workError || unitResolutionError) {
    return (
      <div className="flex h-dvh items-center justify-center bg-black text-white">
        <div className="mx-4 max-w-sm rounded-2xl border border-white/[0.08] bg-zinc-900/95 p-8 text-center">
          <p className="text-lg font-medium">{t.reader.loadError || "加载失败"}</p>
          <p className="mt-2 text-sm text-white/50">
            {workError || unitResolutionError}
          </p>
          <button
            onClick={() => router.replace(
              `/work/${encodeURIComponent(readerNavigation.workId)}`,
            )}
            className="mt-5 rounded-xl bg-accent px-5 py-2.5 text-sm font-medium text-white"
          >
            {t.reader.goBack}
          </button>
        </div>
          ...
          <div className="fixed top-0 right-0 z-[59] h-full w-[85vw] max-w-80 overflow-y-auto bg-zinc-900/95 shadow-2xl backdrop-blur-xl animate-in slide-in-from-right duration-200">
            ...
            <div className="p-2 space-y-1">
              {seriesVolumes.map((vol) => {
                const progress = calculateStoredReadingProgress(
                  vol.lastReadPage || 0,
                  vol.pageCount,
                  vol.lastReadAt || null,
                  vol.readingStatus || "",
                );
                const isCurrent = vol.id === activeUnit?.id;
                return (
                  <button
                    key={vol.id}
                    onClick={async () => {
                      ...
                      if (!isCurrent) {
                        await finishSessionRef.current?.();
                        router.replace(
                          buildUnitReaderPath(readerNavigation.workId, vol),
                        );
                      }
                    }}
                  ...
                  >
                    ...
                    <div className="relative h-12 w-9 shrink-0 overflow-hidden rounded-lg bg-white/5">
                      ...
                      <img
                        src={vol.coverUrl || apiPath(`/api/comics/${vol.comicId}/thumbnail`)}
                        alt={vol.title}
