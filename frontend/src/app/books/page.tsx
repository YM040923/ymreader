...
import { getWorkComicIds, indexWorksByComicId, workToComic } from "@/lib/work-model";
import { batchWorkOperation, fetchAllWorks, reorderWorks } from "@/api/works";
...
export default function BooksPage() {
  ...
  const handleAiSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const index = indexWorksByComicId(allWorks);
        const seen = new Set<string>();
        setAiSearchResults(raw.flatMap((result: {comicId:string;title:string;score:number;reason:string;matchedOn:string[]}) => {
          const work = index.get(result.comicId);
          if (!work || seen.has(work.id)) return [];
          seen.add(work.id);
          return [{ ...result, comicId: work.id, title: work.title }];
        }));
      }
    } catch { /* ignore */ }
  ...
  const { works: apiWorks, loading, fetching, total: apiTotal, totalPages, refetch } = useWorks({
    page: currentPage,
    pageSize,
  ...
  const useRealData = apiTotal > 0 || apiWorks.length > 0 || initializedRef.current;
  if (useRealData && !initializedRef.current) {
    initializedRef.current = true;
  }
  const displayComics: Comic[] = useMemo(() => {
    return apiWorks.map(workToComic);
  }, [apiWorks]);
  const workById = useMemo(() => new Map(apiWorks.map((work) => [work.id, work])), [apiWorks]);
  const comicIdsFor = useCallback((ids: Iterable<string>) => Array.from(ids)
    .flatMap((id) => {
      const work = workById.get(id);
      return work ? getWorkComicIds(work) : [];
    }), [workById]);
  const worksFor = useCallback(
    (ids: Iterable<string>) =>
      Array.from(ids)
  ...
  const handleBatchDelete = useCallback(async (deleteFiles?: boolean) => {
    ...
    // 等动画播完后再真正删除
    setTimeout(async () => {
      await batchWorkOperation("delete", worksFor(ids), deleteFiles ? { deleteFiles: true } : {});
      setRemovingIds(new Set());
      exitBatchMode();
      await refetch();
    }, 400);
  }, [selectedIds, exitBatchMode, refetch, worksFor]);

  const handleBatchFavorite = useCallback(async () => {
    await batchWorkOperation("favorite", worksFor(selectedIds), { isFavorite: true });
    exitBatchMode();
    await refetch();
  }, [selectedIds, exitBatchMode, refetch, worksFor]);

  const handleBatchUnfavorite = useCallback(async () => {
    await batchWorkOperation("unfavorite", worksFor(selectedIds));
    exitBatchMode();
    await refetch();
  }, [selectedIds, exitBatchMode, refetch, worksFor]);
  ...
  const handleBatchAddTags = useCallback(
    async (tags: string[]) => {
      await batchWorkOperation("addTags", worksFor(selectedIds), { tags });
      exitBatchMode();
      await refetch();
    },
    [selectedIds, exitBatchMode, refetch, worksFor]
  ...
  const handleAIBatchSuggestTags = useCallback(async () => {
    const ids = comicIdsFor(selectedIds);
  ...
  const handleBatchSetCategory = useCallback(
    async (categorySlugs: string[]) => {
      await batchWorkOperation("setCategory", worksFor(selectedIds), { categorySlugs });
      exitBatchMode();
      await refetch();
      refetchCategories();
    },
    [selectedIds, exitBatchMode, refetch, refetchCategories, worksFor]
  );


  const handleBatchRemoveTags = useCallback(
    async (tags: string[]) => {
      await batchWorkOperation("removeTags", worksFor(selectedIds), { tags });
      exitBatchMode();
      await refetch();
    },
    [selectedIds, exitBatchMode, refetch, worksFor]
  ...
  const handleBatchSetReadingStatus = useCallback(
    async (status: string) => {
      await batchWorkOperation("setReadingStatus", worksFor(selectedIds), { readingStatus: status });
      exitBatchMode();
      await refetch();
    },
    [selectedIds, exitBatchMode, refetch, worksFor]
  ...
  const handleDragEnd = useCallback(async () => {
    ...
    const orderedWorks = items
      .map((comic) => workById.get(comic.id))
      .filter((work): work is Work => Boolean(work));
    ...
    await reorderWorks(orderedWorks);
    await refetch();
  }, [dragId, dragOverId, sortedComics, refetch, workById]);
  ...
  return (
    <div className="min-h-screen overflow-x-hidden" style={{ background: "var(--theme-background)" }}>
      ...
      <Navbar
        ...
        onUpload={hasManageableLibrary ? handleUpload : undefined}
        uploading={uploading}
              ...
              >
                {sortedComics.map((comic, index) => (
                    ...
                    <ComicCard
                      ...
                      posterAspect={comic.coverAspectRatio && comic.coverAspectRatio > 0 ? String(comic.coverAspectRatio) : undefined}
        ...
        <ComicContextMenu
          ...
          onRead={(id) => {
            ...
            if (c) {
              ...
              router.push(c.detailHref || (isNovel ? `/novel/${id}` : `/reader/${id}`));
            }
          }}
          onDetail={() => router.push(contextMenu.comic.detailHref || `/work/${contextMenu.comic.id}`)}
          onToggleFavorite={async (id) => {
            try {
              await batchWorkOperation(
                contextMenu.comic.isFavorite ? "unfavorite" : "favorite",
                worksFor([id]),
                { isFavorite: !contextMenu.comic.isFavorite },
              );
              await refetch();
            } catch {
              toast.error(t.contextMenu?.favoriteFailed || "收藏操作失败");
            }
          }}
          onDelete={async (id) => {
            ...
            setTimeout(async () => {
              try {
                await batchWorkOperation("delete", worksFor([id]));
                await refetch();
                toast.success(t.comicDetail?.deleteSuccess || "删除成功");
              } catch (reason) {
