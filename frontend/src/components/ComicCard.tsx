...
function getReaderUrl(comic: Comic): string {
  if (comic.readHref) return comic.readHref;
...
function getDetailUrl(comic: Comic): string {
  return comic.detailHref || `/comic/${comic.id}`;
}
...
const ComicCard = memo(function ComicCard({
  ...
  const coverAspectClass = posterAspect ? "" : "aspect-[5/7]";
  const dynamicAspect = posterAspect || (comic.coverAspectRatio && comic.coverAspectRatio > 0 ? String(comic.coverAspectRatio) : undefined);
  const coverObjectFit = comic.workId ? "object-contain" : "object-cover"; // Use cover to fill container properly
      ...
      {batchMode ? (
        <div onClick={handleClick} className="cursor-pointer">
          ...
          >
            ...
            <div className={`relative ${coverAspectClass} w-full overflow-hidden`} style={dynamicAspect ? { aspectRatio: dynamicAspect } : undefined}>
...
<div className="flex flex-wrap items-center gap-1.5">
        ...
        <>
          ...
          <div className={`motion-cover relative overflow-hidden rounded-xl bg-card/70 backdrop-blur-sm border border-white/[0.04] group-hover:shadow-accent/10 group-hover:border-white/[0.08] ${compact ? "shadow-sm" : ""}`}>
            ...
            >
              <div className={`relative ${coverAspectClass} w-full overflow-hidden ${isLandscape ? "bg-black/5 dark:bg-white/5" : ""}`} style={dynamicAspect ? { aspectRatio: dynamicAspect } : undefined}>
                {/* 骨架屏加载占位 */}
                {!coverLoaded && (
