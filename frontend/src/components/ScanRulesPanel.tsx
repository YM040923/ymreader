...
export function ScanRulesPanel() {
  ...
  const applyRecommendedPreset = () => {
    ...
    setMessage({ type: "info", text: "已套用推荐配置：新增入库后自动识别，并用硬链接生成整理目录。确认无误后点击保存。" });
  };
    ...
    try {
      ...
      if (data?.result) {
        ...
        setMessage({
          type: "success",
          text: dryRun
            ? `预览完成（共 ${data.result.total} 项；预计处理 ${data.result.inferred + data.result.directoryOrganized} 项）`
            : `执行完成（识别 ${data.result.inferred}，目录整理 ${data.result.directoryOrganized}，失败 ${data.result.failed}）`,
        });
      }
    } catch (e) {
