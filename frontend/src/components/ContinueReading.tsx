...
export function ContinueReading({ contentType, showTitle = true }: { contentType?: string; showTitle?: boolean }) {
  ...
  const fetchRecent = useCallback(async () => {
    try {
      const [workResponse, novels] = await Promise.all([
        contentType === "novel"
          ? Promise.resolve([])
      ...
      const items = mergeNovelAndWorkItems(
        novels.filter((novel) => Boolean(novel.lastReadAt) && novel.readingStatus !== "finished").map(apiComicToComic),
        workResponse.filter((work) => Boolean(work.lastReadAt) && work.readingStatus !== "finished").map(workToComic),
      )
        .sort((left, right) => new Date(right.lastReadAt || 0).getTime() - new Date(left.lastReadAt || 0).getTime())
        .slice(0, 8);
      setRecentComics(items);
    } catch {
      // ignore
    } finally {
      setLoading(false);
          ...
          <div className="hidden sm:block">
            <div className="flex items-center justify-center gap-3 lg:gap-4 px-4">
                  ...
                  return (
                    ...
                    >
                      <div style={{ width: w }} className="relative">
                        <div
                          className="relative w-full overflow-hidden rounded-xl bg-card shadow-md transition-all duration-300 group-hover:scale-105 group-hover:opacity-100"
                          style={{ aspectRatio: comic.coverAspectRatio && comic.coverAspectRatio > 0 ? String(comic.coverAspectRatio) : "5 / 7" }}
                        >
                          <NSFWCoverGuard
                            src={comic.coverUrl}
                            alt={comic.title}
              ...
              {(() => {
                ...
                const progress = (comic.pageCount || 0) > 0
                  ? calculateStoredReadingProgress(comic.lastReadPage || 0, comic.pageCount || 0, comic.lastReadAt || null, comic.readingStatus)
                  : 0;
                const href = comic.readHref || comic.detailHref || `/work/${comic.id}`;
                  ...
                  <Link key={`active-${comic.id}`} href={href} className="group cursor-pointer relative z-10">
                    <div style={{ width: 230 }} className="relative">
                      <div
                        ...
                        style={{
                          aspectRatio: comic.coverAspectRatio && comic.coverAspectRatio > 0 ? String(comic.coverAspectRatio) : "5 / 7",
                          boxShadow: [
                      ...
                      >
                        ...
                        <div className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-black/85 via-black/40 to-transparent p-4 pt-12">
                          ...
                          <div className="mt-1.5 flex items-center gap-2 text-[11px] text-white/60">
                            ...
                            {(comic.pageCount || 0) > 0 && (
                              <span className="ml-auto">{getReadingPageNumber(comic.lastReadPage || 0, comic.pageCount || 0)}/{comic.pageCount || 0} {t.dashboard?.pages || "页"}</span>
                            )}
                          </div>
              ...
              {recentComics.map((comic) => {
                const progress =
                  (comic.pageCount || 0) > 0
                    ? calculateStoredReadingProgress(comic.lastReadPage || 0, comic.pageCount || 0, comic.lastReadAt || null, comic.readingStatus)
                    : 0;
                const href = comic.readHref || comic.detailHref || (isNovel(comic) ? `/novel/${comic.id}` : `/work/${comic.id}`);

                return (
                  <Link key={comic.id} href={href} className="group shrink-0 snap-center">
                    <div className="w-[140px] space-y-2">
                      <div
                        className="relative w-full overflow-hidden rounded-2xl bg-card/60 backdrop-blur-sm border border-border/30 cover-glow"
                        style={{ aspectRatio: comic.coverAspectRatio && comic.coverAspectRatio > 0 ? String(comic.coverAspectRatio) : "5 / 7" }}
                      >
                        <NSFWCoverGuard
                          src={comic.coverUrl}
